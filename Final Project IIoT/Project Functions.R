...
Forecast.Electric.Demand <- function(Raw_Data)
{
  ...
  # Holidays <- as.numeric(!isHoliday(timeDate(Raw_Data$DATE)))
  ...
  pres.day <- getNthDayOfWeek(third, Mon, Feb, 2017)
  pres.day.idx <- grep(pres.day, Time.Stamp)
  Holidays[pres.day.idx] <- 1

  # Choose Data to Process
  Dependent.Ix <- c(2:4) # Select dependent columns
  Dependent.Data <- cbind(Hours.Modified, Day.Number.Modified, Holidays, Raw_Data[TrainRange,Dependent.Ix]); # X ()
  ...
  Norm.Independent.Data <- normalize(Independent.Data)
  ...
  inputs <- Norm.Dependent.Data # Actual Consumption - used for training
  # targets <- Norm.Independent.Data # Expected Consumption (Regression data) used as Tags
