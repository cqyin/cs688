...
# broken in tm v0.7.5
corpus.txt <- VCorpus(DataframeSource(data.frame(email.texts)))
...
plot(cl,main="Jaccard Distance",xlab="Term Clustering Dendrogram using") # Set plot title

# plot.new()
# plot(cl, hang=-1)
# groups <- cutree(cl, k=3)   # "k=" defines the number of clusters you are using   
...
dtm1 <- as.matrix(DocumentTermMatrix(corpus.temp))
d <- dist(dtm1, method="binary") # Find distance between terms in dtm
cls <- hclust(as.dist(d)) # Perform clustering
cls$labels=doc # Assign labels (terms used) to cluster leaves
plot(cls,main="Jaccard Distance with Stemming",xlab="Term Clustering Dendrogram using") # Set plot title
rect.hclust(cls, k=5, border="red") # draw dendogram with red borders around the 5 clusters

# Hierarchical clustering
library(stringdist)
d <- stringdistmatrix(doc, doc) # Pairwise string distances (optimal string alignment)
cl2 <- hclust(as.dist(d)) # Perform hierarchical clustering
cl2$labels=doc # Assign labels to cluster leaves
plot(cl2,main="Optimal String Alignment",xlab="Term Clustering Dendrogram using")
